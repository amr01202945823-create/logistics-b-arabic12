...
import { PrintIcon, TrashIcon, PlusIcon, DocumentTextIcon, FolderOpenIcon, BookmarkIcon, SearchIcon, EditIcon, XIcon, CheckCircleIcon, DownloadIcon } from '../components/icons';
...
export const InvoiceGeneratorPage: React.FC = () => {
    ...
    const handleSaveDraft = () => {
        const draft = { invoiceData, items, currency };
    ...
    return (
        <div className="max-w-6xl mx-auto pb-20">
            ...
            {/* Header Actions */}
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 no-print">
                ...
                <div className="flex gap-2">
                    <Button onClick={handleSaveDraft} variant="secondary"><BookmarkIcon className="w-4 h-4 me-2"/> {t('saveDraft')}</Button>
                    <Button onClick={handleLoadDraft} variant="secondary"><FolderOpenIcon className="w-4 h-4 me-2"/> {t('loadDraft')}</Button>
                    <Button onClick={() => window.print()} variant="secondary"><PrintIcon className="w-5 h-5 me-2"/> {t('printOrSave')}</Button>
                    <Button onClick={handleDownloadPdf} className="bg-blue-600 hover:bg-blue-700 text-white"><DownloadIcon className="w-5 h-5 me-2"/> Download PDF</Button>
                </div>
            </div>
            ...
            <div className="space-y-6 mb-12 no-print animate-fade-in">
                ...
                {/* Integration Bar */}
                <Card className="card-shadow bg-blue-50 border-blue-100">
                    <CardContent className="p-4 flex items-center gap-4">
                        <SearchIcon className="text-blue-600 w-5 h-5" />
                        <Input 
                        ...
                        />
                        <Button onClick={handleLoadShipment} variant="secondary" size="sm">{t('loadFromShipment')}</Button>
                    </CardContent>
                </Card>
