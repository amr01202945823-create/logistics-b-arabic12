...
import { ArrowLeftIcon, PlusIcon, TruckIcon, ShipIcon, CheckBadgeIcon, UserCircleIcon, CubeIcon, CalendarDaysIcon, ThermometerIcon, DocumentTextIcon, PhoneIcon, EditIcon, CurrencyDollarIcon, ChatBubbleBottomCenterTextIcon, TrashIcon, ArrowRightIcon, SearchIcon } from '../components/icons';
...
const AddShipmentModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newShipment: Shipment = {
            ...
            customer: formData.importer || 'Unknown', 
            weight: formData.weight,
            value: formData.value,
            status: 'pending',
            createdAt: new Date().toISOString(),
            updates: [{ timestamp: new Date().toISOString(), message: 'Shipment created' }],
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={t('addShipment')}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <Input name="id" placeholder={t('shipmentId')} value={formData.id} onChange={handleChange} required />
                <div className="grid grid-cols-2 gap-4">
                    <Input name="origin" placeholder={t('origin')} value={formData.origin} onChange={handleChange} required />
                    <Input name="destination" placeholder={t('destination')} value={formData.destination} onChange={handleChange} required />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <Input name="exporter" placeholder={t('exporterName')} value={formData.exporter} onChange={handleChange} />
                    <Input name="importer" placeholder={t('importerName')} value={formData.importer} onChange={handleChange} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <Input name="customsBroker" placeholder={t('customsBrokerName')} value={formData.customsBroker} onChange={handleChange} />
                    <Input name="transportCompany" placeholder={t('transportCompany')} value={formData.transportCompany} onChange={handleChange} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <Input name="shippingLine" placeholder={t('shippingLine')} value={formData.shippingLine} onChange={handleChange} />
                    <Input name="bookingNo" placeholder={t('bookingNo')} value={formData.bookingNo} onChange={handleChange} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <Input name="cost" placeholder={t('cost')} value={formData.cost} onChange={handleChange} type="number" />
                    <Input name="price" placeholder={t('price')} value={formData.price} onChange={handleChange} type="number" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <Input name="weight" placeholder={t('shipmentWeight')} value={formData.weight} onChange={handleChange} />
                    <Input name="value" placeholder={t('shipmentValue')} value={formData.value} onChange={handleChange} />
                </div>
...
export const ShipmentTrackerPage: React.FC<{
    ...
    const selectedShipment = selectedId ? shipments[selectedId] : null;
    ...
    const handleUpdateShipment = (id: string, newStatus: ShipmentStatus, updateMessage: string, details?: Partial<Shipment>) => {
        onUpdateShipments(prev => {
            ...
            const updated: Shipment = {
                ...
                status: newStatus,
                updates: [
                    { timestamp: new Date().toISOString(), message: updateMessage },
                    ...current.updates
                ]
            };
