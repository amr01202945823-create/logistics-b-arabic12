        ...
        <div className="max-w-7xl mx-auto pb-12">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10 min-h-[400px]">
                {paginatedCompanies.map(company => {
                    ...
                    return (
                        <Card key={company.id} className={`card-shadow-hover cursor-pointer flex flex-col group ${company.isFeatured ? 'border-2 border-yellow-400 shadow-md relative' : ''}`} onClick={() => onCompanyClick(company)}>
                            {company.isFeatured && (
                                <div className="bg-yellow-400 text-yellow-900 text-[10px] font-bold px-3 py-1 text-center w-full uppercase tracking-wider rounded-t-lg">
                                    FEATURED PARTNER
                                </div>
                            )}
