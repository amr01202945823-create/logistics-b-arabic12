    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (user) {
                ...
                // Should not happen if login returns Promise<User> but acting defensively
                setError('Login failed: No user data returned.');
            }
        } catch (err: any) {
            console.error("Login Page Error:", err);
            // Translate common errors if needed, otherwise show message
            let msg = err.message || 'Login failed. Please try again.';
            if (msg === 'Failed to fetch') msg = 'Network error: Unable to connect to server. (Demo mode fallback failed)';
            setError(msg);
        } finally {
            ...
            <Card className="w-full max-w-md card-shadow border border-border">
                ...
                <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        {error && <p className="text-red-600 text-sm text-center bg-red-50 p-3 rounded border border-red-100 animate-pulse">{error}</p>}
                        <div>
