...
import { 
    ...
    ShieldCheckIcon, EnvelopeIcon, PhoneIcon, DocumentTextIcon, CheckBadgeIcon, LockClosedIcon
...
const SidebarItem = ({ id, icon, label, activeTab, onClick, language }: { id: Tab, icon: React.ReactNode, label: string, activeTab: Tab, onClick: (id: Tab) => void, language: string }) => (
    <button 
        onClick={() => onClick(id)}
                    ...
                    <CardContent className="p-6 space-y-4">
                        ...
                        <form onSubmit={handleSave} className="grid gap-4">
                            ...
                            <div>
                                ...
                                <Input name="phone" value={formData.phone} onChange={handleChange} placeholder="+20 123 456 7890" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-text-muted mb-1">{t('companyDetails')}</label>
                                <Input name="companyName" value={formData.companyName} onChange={handleChange} placeholder="Logistics Co." />
                            </div>
                            ...
                            <div>
                                <h4 className="font-bold text-sm text-text-heading mb-3 flex items-center gap-2">
                                    <LockClosedIcon className="w-4 h-4 text-primary" />
                                    {t('securitySettings')}
                                </h4>
                                <div className="grid gap-3 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg border border-border">
                                    <div>
                                        <label className="block text-xs font-medium text-text-muted mb-1">{t('currentPassword')}</label>
                                        <Input 
                                    ...
                                    <div>
                                        <label className="block text-xs font-medium text-text-muted mb-1">{t('newPassword')}</label>
                                        <Input 
                                            name="newPassword" 
                                            type="password" 
                            ...
                            <div className="pt-4 flex items-center gap-4">
                                <Button type="submit" disabled={isSaving}>
                                    {isSaving ? <span className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> {t('loading')}</span> : t('saveChanges')}
                                </Button>
                    ...
                    <CardContent className="p-6 space-y-6">
                        <div className="flex items-center justify-between pb-4 border-b border-border">
                            ...
                            <button 
                                ...
                                className={`relative inline-block w-12 h-6 rounded-full transition-colors ${prefs.emailNotifications ? 'bg-blue-600' : 'bg-gray-300 dark:bg-slate-700'}`}
                            >
                                <span className={`absolute top-1 bg-white w-4 h-4 rounded-full transition-transform transform ${prefs.emailNotifications ? (language === 'ar' ? '-translate-x-6 left-auto right-1' : 'translate-x-6 left-1') : (language === 'ar' ? 'right-1' : 'left-1')}`}></span>
                            </button>
                        </div>
                        <div className="flex items-center justify-between pb-4 border-b border-border">
                            <div>
                            ...
                            <button 
                                ...
                                className={`relative inline-block w-12 h-6 rounded-full transition-colors ${prefs.smsNotifications ? 'bg-blue-600' : 'bg-gray-300 dark:bg-slate-700'}`}
                            >
                                <span className={`absolute top-1 bg-white w-4 h-4 rounded-full transition-transform transform ${prefs.smsNotifications ? (language === 'ar' ? '-translate-x-6 left-auto right-1' : 'translate-x-6 left-1') : (language === 'ar' ? 'right-1' : 'left-1')}`}></span>
                            </button>
                        </div>
                        <div className="flex items-center justify-between">
                            ...
                            <button 
                                ...
                                className={`relative inline-block w-12 h-6 rounded-full transition-colors ${prefs.marketingEmails ? 'bg-blue-600' : 'bg-gray-300 dark:bg-slate-700'}`}
                            >
                                <span className={`absolute top-1 bg-white w-4 h-4 rounded-full transition-transform transform ${prefs.marketingEmails ? (language === 'ar' ? '-translate-x-6 left-auto right-1' : 'translate-x-6 left-1') : (language === 'ar' ? 'right-1' : 'left-1')}`}></span>
                            </button>
                        </div>
                    </CardContent>
                </Card>
                ...
                <div className="lg:col-span-1 order-1 lg:order-2">
                    <div className="sticky top-24 space-y-6">
                        ...
                        <nav className="space-y-1 bg-surface rounded-xl border border-border p-2 shadow-sm">
                            <SidebarItem id="dashboard" icon={<HomeIcon />} label={t('viewDashboard')} activeTab={activeTab} onClick={setActiveTab} language={language} />
                            <SidebarItem id="shipments" icon={<CubeIcon />} label={t('myShipments')} activeTab={activeTab} onClick={setActiveTab} language={language} />
                            <SidebarItem id="routes" icon={<MapPinIcon />} label={t('savedRoutes')} activeTab={activeTab} onClick={setActiveTab} language={language} />
                            <SidebarItem id="settings" icon={<UserCircleIcon />} label={t('accountSettings')} activeTab={activeTab} onClick={setActiveTab} language={language} />
                            <SidebarItem id="notifications" icon={<BellIcon />} label={t('notifications')} activeTab={activeTab} onClick={setActiveTab} language={language} />
                            <SidebarItem id="billing" icon={<CurrencyDollarIcon />} label={t('subscription')} activeTab={activeTab} onClick={setActiveTab} language={language} />
                            <SidebarItem id="help" icon={<QuestionMarkCircleIcon />} label={t('helpAndSupport')} activeTab={activeTab} onClick={setActiveTab} language={language} />
                        </nav>
                        ...
                        <div className="pt-4">
                            ...
                            >
                                <LogOutIcon className={`w-5 h-5 ${language === 'ar' ? '' : 'rotate-180'}`} />
                                <span>{t('logout')}</span>
                            </button>
                        </div>
                    </div>
