...
import type { SavedRoute, Company, Article, Shipment } from '../types';
...
import { 
  ...
  UsersIcon, FileTextIcon, CalendarDaysIcon, EnvelopeIcon, FolderOpenIcon, TrashIcon
...
const RouteMap = ({ origin, destination }: { origin: string, destination: string }) => {
    ...
    // Using Google Maps Embed API in "View" mode (legacy compatible) to visualize the route
    // This provides a visual grounding context for the route planned
    const src = `https://maps.google.com/maps?q=from+${encodeURIComponent(origin)}+to+${encodeURIComponent(destination)}&t=&z=4&ie=UTF8&iwloc=&output=embed`;
    
    return (
        <div className="rounded-xl overflow-hidden border border-gray-200 dark:border-slate-700 shadow-sm mb-8 bg-slate-50 dark:bg-slate-800">
            <div className="bg-gray-100 dark:bg-slate-800 px-4 py-2 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center">
                 <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase flex items-center gap-2">
                    <MapPinIcon className="w-4 h-4 text-red-500"/> Route Visualization
                 </h4>
            </div>
            <iframe
                width="100%"
...
export const RoutePlannerPage: React.FC<{
    ...
    companies: Company[];
    articles: Article[];
}> = ({ onSaveRoute, savedRoutes, onDeleteRoute, onAddShipment, systemPromptOverride, companies, articles }) => {
    ...
    const openSaveModal = () => {
        if (!origin || !destination || !goods) {
            alert(language === 'ar' ? 'يرجى ملء تفاصيل المسار أولاً' : 'Please fill in route details first');
            return;
        }
        setCustomRouteName(`${origin} to ${destination}`);
        setIsSaveModalOpen(true);
    };

    const confirmSaveRoute = () => {
        if (!customRouteName) return;
        onSaveRoute({
            id: uuidv4(),
    ...
    const handleLoadRoute = (route: SavedRoute) => {
        ...
        setGoods(route.goods);
        setIncoterm(route.incoterm);
        setShipmentType(route.shipmentType);
        setQuantity(route.quantity);
        setIsLoadModalOpen(false);
        // Optional: Trigger analysis immediately or let user click analyze
    };
    ...
    return (
        <div className="max-w-5xl mx-auto pb-12">
            ...
            <div className="flex justify-end mb-4 gap-2">
                <Button variant="secondary" onClick={() => setIsLoadModalOpen(true)} className="flex items-center gap-2">
                    <FolderOpenIcon className="w-5 h-5" />
                    {language === 'ar' ? 'تحميل مسار محفوظ' : 'Load Saved Route'}
                </Button>
            </div>
                    ...
                    <div className="grid md:grid-cols-3 relative z-10">
                        ...
                        <div className="p-6 border-b md:border-b-0 md:border-r border-border hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group bg-white dark:bg-slate-900">
                            ...
                            <div className="mt-3 pt-3 border-t border-dashed border-border/50">
                                <label className="flex items-center gap-1.5 text-[10px] font-bold text-text-muted uppercase mb-1.5">
                                    <CalendarDaysIcon className="w-3.5 h-3.5" />
                                    {t('pickupDate')}
                                </label>
                                <Input 
                                    type="date"
                                    className="h-9 px-2 text-sm bg-background border-border/60 focus:ring-1 focus:ring-primary/20"
                                    value={pickupDate}
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl overflow-hidden border border-gray-200 dark:border-slate-700">
                        ...
                        <div className="bg-slate-900 text-white p-6 flex flex-col md:flex-row justify-between items-start md:items-center relative overflow-hidden gap-4">
                            ...
                            <div className="flex gap-2 relative z-10 no-print self-end md:self-auto">
                                ...
                                <Button size="sm" onClick={openSaveModal} className="bg-indigo-600 hover:bg-indigo-500 border-none text-white shadow-lg shadow-indigo-900/50">
            ...
            {/* Save Route Modal */}
            <Modal isOpen={isSaveModalOpen} onClose={() => setIsSaveModalOpen(false)} title={language === 'ar' ? 'حفظ المسار' : 'Save Route'}>
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-text-muted mb-1">{t('routeName')}</label>
                        <Input 
                            value={customRouteName} 
                            onChange={(e) => setCustomRouteName(e.target.value)} 
                            placeholder={language === 'ar' ? 'مثال: مسار الصيف السريع' : 'e.g., Summer Express Route'}
                            autoFocus
                        />
                    </div>
                    <div className="flex justify-end gap-2 mt-4">
                        <Button variant="secondary" onClick={() => setIsSaveModalOpen(false)}>{t('cancel')}</Button>
                        <Button onClick={confirmSaveRoute}>{t('save')}</Button>
                    </div>
                </div>
            </Modal>

            {/* Load Route Modal */}
            <Modal isOpen={isLoadModalOpen} onClose={() => setIsLoadModalOpen(false)} title={language === 'ar' ? 'تحميل مسار محفوظ' : 'Load Saved Route'}>
                <div className="space-y-2 max-h-[60vh] overflow-y-auto">
                    {savedRoutes.length > 0 ? (
                        savedRoutes.map(route => (
                            <div key={route.id} className="flex items-center justify-between p-3 border border-border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group">
                                <button 
                                    onClick={() => handleLoadRoute(route)}
                                    className="flex-grow text-start"
                                >
                                    <h4 className="font-bold text-sm text-text-heading">{route.name}</h4>
                                    <p className="text-xs text-text-muted">{route.origin} → {route.destination} • {route.shipmentType}</p>
                                </button>
                                <button 
                        ...
                        <p className="text-center text-text-muted py-8">{t('noSavedRoutes')}</p>
                    )}
                </div>
            </Modal>
        </div>
